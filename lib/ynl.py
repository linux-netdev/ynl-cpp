...
class NlAttr:
    ...
    def get_format(cls, attr_type, byte_order=None):
        ...
        if byte_order:
            return format.big if byte_order == "big-endian" else format.little
    ...
    def as_auto_scalar(self, attr_type, byte_order=None):
        if len(self.raw) != 4 and len(self.raw) != 8:
            raise Exception(
                f"Auto-scalar len payload be 4 or 8 bytes, got {len(self.raw)}"
            )
...
def _genl_load_families():
    with socket.socket(
        socket.AF_NETLINK, socket.SOCK_RAW, Netlink.NETLINK_GENERIC
    ) as sock:
...
class YnlFamily(SpecFamily):
    def __init__(self, def_path, schema=None, process_unknown=False, recv_size=0):
        ...
        try:
            if self.proto == "netlink-raw":
                self.nlproto = NetlinkProtocol(self.yaml["name"], self.yaml["protonum"])
            else:
        ...
        self.sock = socket.socket(
            socket.AF_NETLINK, socket.SOCK_RAW, self.nlproto.proto_num
        )
    ...
    def ntf_subscribe(self, mcast_name):
        ...
        self.sock.setsockopt(
            Netlink.SOL_NETLINK, Netlink.NETLINK_ADD_MEMBERSHIP, mcast_id
        )
    ...
    def _add_attr(self, space, name, value, search_attrs):
        ...
        if attr["type"] == "nest":
            ...
            for subname, subvalue in value.items():
                attr_payload += self._add_attr(
                    attr["nested-attributes"], subname, subvalue, sub_attrs
                )
        elif attr["type"] == "flag":
            if not value:
            ...
            if attr.is_auto_scalar:
                attr_type = attr["type"][0] + (
                    "32" if scalar.bit_length() <= 32 else "64"
                )
            else:
                attr_type = attr["type"]
            ...
            if msg_format.attr_set:
                if msg_format.attr_set in self.attr_sets:
                    ...
                    for subname, subvalue in value.items():
                        attr_payload += self._add_attr(
                            msg_format.attr_set, subname, subvalue, sub_attrs
                        )
                else:
    ...
    def _decode_array_attr(self, attr, attr_spec):
        ...
        while offset < len(attr.raw):
            ...
            if attr_spec["sub-type"] == "nest":
                subattrs = self._decode(
                    NlAttrs(item.raw), attr_spec["nested-attributes"]
                )
                ...
                raise Exception(
                    f'Unknown {attr_spec["sub-type"]} with name {attr_spec["name"]}'
                )
        return decoded
    ...
    def _resolve_selector(self, attr_spec, search_attrs):
        ...
        if value not in sub_msg_spec.formats:
            raise Exception(
                f"No message format for '{value}' in sub-message spec '{sub_msg}'"
            )
        ...
        for attr in attrs:
            try:
                ...
                if not self.process_unknown:
                    raise Exception(
                        f"Space '{space}' has no attribute with value '{attr.type}'"
                    )
            ...
            if attr_spec["type"] == "nest":
                subdict = self._decode(
                    NlAttrs(attr.raw), attr_spec["nested-attributes"], search_attrs
                )
                decoded = subdict
                ...
                if not self.process_unknown:
                    raise Exception(
                        f'Unknown {attr_spec["type"]} with name {attr_spec["name"]}'
                    )
    ...
    def _decode_extack_path(self, attrs, attr_set, offset, target):
        for attr in attrs:
            try:
                ...
                raise Exception(
                    f"Space '{attr_set.name}' has no attribute with value '{attr.type}'"
                )
            if offset > target:
                break
            if offset == target:
            ...
            if attr_spec["type"] != "nest":
                raise Exception(
                    f"Can't dive into {attr.type} ({attr_spec['name']}) for extack"
                )
            offset += 4
            subpath = self._decode_extack_path(
                NlAttrs(attr.raw),
    ...
    def _decode_extack(self, request, op, extack):
        ...
        path = self._decode_extack_path(
            msg.raw_attrs, op.attr_set, offset, extack["bad-attr-offs"]
        )
        if path:
        ...
        for m in members:
            ...
            if m.type == "pad":
                ...
                if m.struct:
                    ...
                    value = self._decode_struct(data[offset : offset + len], m.struct)
                    offset += len
                else:
